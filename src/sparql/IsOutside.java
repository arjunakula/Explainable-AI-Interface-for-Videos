...
public class IsOutside extends FunctionBase
{

	public IsOutside() { super() ; }
    ...
    public final NodeValue exec(List<NodeValue> args) 
    {
    	...
    	{
    		System.out.println("DEBUG IsOutside running");
    	}
        ...
        {	if (MseeFunction.IsObjectType(argType_list.get(i)))
        	...
        	{  throw new ARQInternalErrorException(Utils.className(this)+": IsOutside: unable to parse arguments") ;
        	}
        }
		...
		for (CObjectData object_agent : list_agent)
			...
			{		
						...
						(object_agent.id.compareTo(object_patient.id)!=0))
				{
					...
					if (bTimeLocOkay)
					{
						b = SpatialFunctions.IsOutside(time_data, loc_data, object_agent, object_patient);
						if (b)
							break;
					}
				}	
		...
		*/ 		
		...
		if (MseeFunction.bVerbose)
    	{
			System.out.println("DEBUG IsOutside ending " + b);
    	}
