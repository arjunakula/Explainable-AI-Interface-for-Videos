...
public class IsFacingOpposite extends FunctionBase
{
	
	public IsFacingOpposite() { super() ; }
    ...
    public final NodeValue exec(List<NodeValue> args) 
    {
    	...
    	{
    		System.out.println("DEBUG IsFacingOpposite running");
    	}
        ...
        {	if (MseeFunction.IsObjectType(argType_list.get(i)))
        	...
        	{  throw new ARQInternalErrorException(Utils.className(this)+": IsFacingOpposite: unable to parse arguments") ;
        	}
        }
		...
		for (CObjectData object_agent : list_agent)
			...
			{		
						...
						(object_agent.id.compareTo(object_patient.id)!=0))
				{
					...
					if (bTimeLocOkay)
					{
						b = SpatialFunctions.IsFacingOpposite(time_data, loc_data, object_agent, object_patient);
						if (b)
							break;
					}
				}	
		...
		*/ 		
		...
		if (MseeFunction.bVerbose)
    	{
			System.out.println("DEBUG IsFacingOpposite ending " + b );
    	}
