...
public class IsEntering extends FunctionBase
{

	public IsEntering() { super() ; }
    ...
    public final NodeValue exec(List<NodeValue> args) 
    {
    	...
    	{
    		System.out.println("DEBUG IsEntering running");
    	}
		...
		for (CObjectData object_agent : list_agent)
			...
			{		
				if ((false == MseeFunction.IsTwoObjectsHaveSameObsId(object_agent, object_patient)) ||
					(false == MseeFunction.IsTwoObjectsBothAtTime(object_agent, object_patient, time_data)))
				...
				//System.out.println("SpatialFunctions.IsEntering " + object_agent.id);
				b = SpatialFunctions.IsEntering(time_data, loc_data, object_agent, object_patient);
				if (b)
					break;
			}
		...
		*/ 
		...
		if (MseeFunction.bVerbose)
    	{
			System.out.println("DEBUG IsEntering ending " + b);
    	}
