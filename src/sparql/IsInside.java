...
public class IsInside extends FunctionBase
{

	public IsInside() { super() ; }
    ...
    public final NodeValue exec(List<NodeValue> args) 
    {
    	...
    	{
    		System.out.println("DEBUG IsInside running");
    	}
        ...
        {	if (MseeFunction.IsObjectType(argType_list.get(i)))
        	...
        	{  throw new ARQInternalErrorException(Utils.className(this)+": IsInside: unable to parse arguments") ;
        	}
        }
		...
		ArrayList<CObjectData> list_agent = MseeDataset.FindObjectDataList(agent_id);
		ArrayList<CObjectData> list_patient = MseeDataset.FindObjectDataList(patient_id);
		...
		for (CObjectData object_agent : list_agent)
			...
			{		
				...
				{
					if (agent_id.contains("SDT_SDT4_Human"))   // agent is SDT defined
					{
						b = true;
					}
					else
					{
						...
						if (bTimeLocOkay)
						{
							...
							b = SpatialFunctions.IsInside(time_data, loc_data, object_agent, object_patient);
						}
					}
					if (b)
					{	break;
		...
		*/
		...
		if (MseeFunction.bVerbose)
    	{
			System.out.println("DEBUG IsInside ending " +b );
    	}
