...
public class IsBelow extends FunctionBase
{

	public IsBelow() { super() ; }
    ...
    public final NodeValue exec(List<NodeValue> args) 
    {
    	...
    	{
    		System.out.println("DEBUG IsBelow running");
    	}
        ...
        {	if (MseeFunction.IsObjectType(argType_list.get(i)))
        	...
        	{  throw new ARQInternalErrorException(Utils.className(this)+": IsBelow: unable to parse arguments") ;
        	}
        }
		...
		for (CObjectData object_agent : list_agent)
			for (CObjectData object_patient : list_patient)
			{
				...
				if (false == MseeFunction.IsTwoObjectsBothAtTimeLocation(object_agent, object_patient, time_data, loc_data))
				{
					continue;
				}				
						...
						(object_agent.id.compareTo(object_patient.id)!=0))
				{
					...
					if (bTimeLocOkay)
					{
						b = SpatialFunctions.IsBelow(time_data, loc_data, object_agent, object_patient);
						if (b)
							break;
					}
				}	
		...
		if (MseeFunction.bVerbose)
    	{
			System.out.println("DEBUG IsBelow ending");
    	}
