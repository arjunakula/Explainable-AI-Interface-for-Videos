...
public class IsDismounting extends FunctionBase
{

	public IsDismounting() { super() ; }
    ...
    public final NodeValue exec(List<NodeValue> args) 
    {
    	...
    	{
    		System.out.println("DEBUG IsDismounting running");
    	}
        ...
        {	if (MseeFunction.IsObjectType(argType_list.get(i)))
        	...
        	{  throw new ARQInternalErrorException(Utils.className(this)+": IsDismounting: unable to parse arguments") ;
        	}
        }
				...
				(object_agent.id.compareTo(object_patient.id)!=0))
		{
			...
			if (bTimeLocOkay)
			{
				b = SpatialFunctions.IsDismounting(time_data, loc_data, object_agent, object_patient);
			}
		}		
		...
		if (MseeFunction.bVerbose)
    	{
			System.out.println("DEBUG IsDismounting ending" + b);
    	}
