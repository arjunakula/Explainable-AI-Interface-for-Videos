...
public class IsDriving extends FunctionBase
{

	public IsDriving() { super() ; }
    ...
    public final NodeValue exec(List<NodeValue> args) 
    {
    	...
    	{
    		System.out.println("DEBUG IsDriving running");
    	}
        ...
        {	if (MseeFunction.IsObjectType(argType_list.get(i)))
        	...
        	{  throw new ARQInternalErrorException(Utils.className(this)+": IsDriving: unable to parse arguments") ;
        	}
        }
		...
		CObjectData object_agent = MseeDataset.FindObjectData(agent_id);
				...
				(object_agent.id.compareTo(object_patient.id)!=0))
		{
			...
			if (bTimeLocOkay)
			{
				...
				{
					b = false;				// SDT 6, person is not driving 
				}
				else
				{
					b = SpatialFunctions.IsDriving(time_data, loc_data, object_agent, object_patient);
				}
			}	
		}		
		...
		if (MseeFunction.bVerbose)
    	{
			System.out.println("DEBUG IsDriving ending " + b);
    	}
